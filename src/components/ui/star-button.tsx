  ...
  return (
    <button
      style={{
        ...
        isolation: "isolate",
        background: "linear-gradient(135deg, #4338ca 0%, #6366f1 50%, #312e81 100%)",
      } as CSSProperties}
      ref={pathRef}
      className={cn(
        "relative z-[3] overflow-hidden h-10 px-5 py-2 inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-full text-sm font-semibold transition-all hover:brightness-110 disabled:pointer-events-none disabled:opacity-50",
        className,
      )}
      {...props}
    >
      {/* travelling light */}
      <div
        className="absolute inset-0 opacity-60 animate-star-btn bg-[radial-gradient(circle_at_center,var(--light-color)_0%,transparent_45%)]"
        style={{
          offsetPath: "var(--path)",
          offsetDistance: "0%",
      ...
      />
      {/* border overlay with star pattern */}
      <div
        className="absolute inset-0 z-[4] overflow-hidden rounded-[inherit] opacity-10"
        style={{ borderWidth: "var(--border-width)", borderColor: "rgba(255,255,255,0.2)", borderStyle: "solid" }}
        aria-hidden="true"
      >
        <StarBackground color="rgba(255,255,255,0.4)" />
      </div>
      {/* label */}
      <span className="z-10 relative text-white tracking-wide">{children}</span>
    </button>
  );
}
