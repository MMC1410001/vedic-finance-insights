...
const Starfield: React.FC<StarfieldProps> = ({
    ...
    if (
      ...
      if (!sd.current.star.arr.length) {
        ...
        sd.current.star.arr = sd.current.star.arr.map((star, i) => {
          ...
          newStar[3] =
            sd.current.x + (newStar[0] / newStar[2]) * ratio;
          newStar[4] =
            sd.current.y + (newStar[1] / newStar[2]) * ratio;
          return newStar;
        });
      }
      if (sd.current.ctx) {
  ...
  const update = () => {
    mouse.current.x =
      (cursor.current.x - sd.current.x) / easing;
    mouse.current.y =
      (cursor.current.y - sd.current.y) / easing;
  ...
  const draw = () => {
    ...
    sd.current.star.arr.forEach((star) => {
      if (
        ...
        ctx.lineWidth =
          (1 - sd.current.star.colorRatio * star[2]) * 2;
        ctx.beginPath();
  ...
  const mouseHandler = (event: MouseEvent) => {
    ...
    if (el) {
      cursor.current.x =
        event.pageX ||
        event.clientX + el.scrollLeft - el.clientLeft;
      cursor.current.y =
        event.pageY ||
        event.clientY + el.scrollTop - el.clientTop;
    }
  };
