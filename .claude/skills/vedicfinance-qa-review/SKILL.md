...
The doc names a **revenue-critical subset** for a 2-day sprint: rows 1, 2, 6, 9, 10, 11–14, 16, 20. If time is short, that's the cut. It also flags rows 9, 10, 20, 21 as the best Playwright candidates — e2e is unblocked, so those are now worth writing rather than doing by hand.
...
Both suites now exist and both run. `npm test` is 33 Vitest files; `npm run test:e2e` is 97
Playwright specs across 8 files in `e2e/` (the config that imported the missing
`lovable-agent-playwright-config` was replaced). Neither is a release verdict on its own.

**Read an e2e failure carefully before reporting it.** The suite is `workers: 1` against the
single production Supabase project and takes ~20 minutes, so its 30s waits time out under
load: a failure that says "never rendered" or "never finished loading" is usually the
harness, not the app. Re-run it in isolation, and drive the page by hand before ranking it
alongside a real defect.

Say plainly which findings are machine-verified and which need a human.
...
No `prefers-reduced-motion` support anywhere. Check whether the diff makes it worse; don't present it as a new discovery.

The note that used to sit here — "visitor analytics is written but not deployed, `visitor_events` does not exist" — is **stale**. That pipeline is live and collecting, and /admin → Analytics reads real rows. The dev sample data only appears when a query returns nothing AND `import.meta.env.DEV` is set, behind a banner saying so.
